...
alias glog='git log --graph --decorate --abbrev-commit --date=short --pretty=format:"%C(${_hex_gold})%h%C(reset) %s %C(${_hex_iris})<%an>%C(reset) %C(${_hex_muted})%ad%C(reset)%C(auto)%d%C(reset)"'
